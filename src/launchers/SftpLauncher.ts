...
export class SftpLauncher extends BaseLauncher {
  ...
  async launch(options: ConnectionOptions): Promise<void> {
    if (!this.isMacOS()) {
      throw new Error('SFTP launcher currently only supports macOS');
    }
  ...
  private async launchInTerminalApp(
    ...
    if (useSshpass && options.password) {
      ...
      command = `sshpass -f '${passFile}' ${sftpCommand}; rm -f '${passFile}'`;
    } else {
      command = sftpCommand;
    }
  ...
  private launchInIntegratedTerminal(
    ...
    if (useSshpass && options.password) {
      ...
      terminal.show();
      terminal.sendText(`sshpass -e ${sftpCommand}`);
    } else {
      // Regular SFTP without password
      const terminal = vscode.window.createTerminal(terminalName);
      terminal.show();
      terminal.sendText(sftpCommand);
    }
  }
